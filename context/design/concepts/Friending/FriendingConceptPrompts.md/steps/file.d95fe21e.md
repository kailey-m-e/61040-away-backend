---
timestamp: 'Sat Oct 18 2025 15:02:14 GMT-0400 (Eastern Daylight Time)'
parent: '[[..\20251018_150214.51b87cf5.md]]'
content_id: d95fe21e0d180fb3ca480504349dc0de4577016569b9f08c98ac7f10942ebe8b
---
...
export default class FriendingConcept {
  ...
  async unrequestFriend(
    ...
    const result: FindAndModifyResultAlias<UsersDoc> = await this.users
      .findOneAndUpdate(
  ...
  async acceptFriend(
    ...
    return await this._runInTransaction(async (session) => {
      ...
      const friendRequestPulled: FindAndModifyResult<UsersDoc> = await this
  ...
  async rejectFriend(
    ...
    // find user, ensure user has outgoing request from friend,
    // and atomically remove request
    const result: FindAndModifyResult<UsersDoc> = await this.users
      .findOneAndUpdate(
        { _id: friend, outgoingRequests: user },
        { $pull: { outgoingRequests: user } },
        { returnDocument: "before" },
  ...
  async endFriendship(
    ...
    return await this._runInTransaction(async (session) => {
      ...
      const userFriendshipPulled: FindAndModifyResult<UsersDoc> = await this
        .users.findOneAndUpdate(
