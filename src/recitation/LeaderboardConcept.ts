...
export default class LeaderboardConcept {
  ...
  async createPlayer(
    ...
    const existingPlayer = await this.players.findOne({ name });
    if (existingPlayer) {
      return { error: "Player with this name already exists" };
    }
    const newPlayerId = freshID() as Player;
  ...
  async _getPlayerScores({ player }: { player: Player }): Promise<ScoreDoc[]> {
    ...
    return await this.scores.find({ player }).toArray();
  }
  ...
  async _getTopScores({ limit }: { limit: number }): Promise<ScoreDoc[]> {
    ...
    return await this.scores.find().sort({ value: -1 }).limit(limit).toArray();
  }
  ...
  async _getScoresSince({ date }: { date: Date }): Promise<ScoreDoc[]> {
    ...
    return await this.scores.find({ submittedAt: { $gte: date }}).toArray();
  }
  ...
  async _getPlayerStats(
    ...
    const pipeline = [
    {
    $match: {player: player,}, // Stage 1: Filter documents to only this player's scores.
    },
    {
    $group: { // Stage 2: Group the matched documents and calculate stats.
    _id: null, // Group all documents into a single output document.
    totalScores: { $count: {} },
    highestScore: { $max: "$value" },
    averageScore: { $avg: "$value" },
    },
    },
    {
    $project: { // Optional Stage 3: Clean up the output to match our PlayerStats interface.
    ...
    const result = await this.scores.aggregate(pipeline).toArray();
    if (result.length === 0) { // If the player has no scores, the result array will be empty.
    return null;
    }
    return result[0] as PlayerStats; // Otherwise, return the first (and only) document.
    }
