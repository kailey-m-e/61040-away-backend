...
export default class FriendingConcept {
  ...
  async endFriendship(
    ...
    return await this._runInTransaction(async (session) => {
      ...
      // atomically remove user from friend's friends
      await this.users.updateOne(
        { _id: friend },
        { $pull: { friends: user } },
        { session },
      ...
      // atomically remove friend from user's friends
      await this.users.updateOne(
        { _id: user },
        { $pull: { friends: friend } },
        { session },
  ...
  /**
   * Query: Retrieves all users who have requested given user as a friend.
  ...
  /**
   * Query: Retrieves all friends that given user has an outgoing request for.
  ...
  /**
   * Query: Retrieves all friends of given user.
