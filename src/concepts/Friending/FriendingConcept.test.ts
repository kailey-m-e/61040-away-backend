...
/**
 * Test Case 6
 * Demonstrates concurrency sceanrios; concurrent mutual friend requests, concurrent accept
 * and re-request, concurrent ending of friendship
 */
Deno.test("Test Case 7 - concurrecy", async () => {
  const [db, client] = await testDb();
  const friendingConcept = new FriendingConcept(db, client);
  ...
  try {
    ...
    // Expect: At least one `endFriendship` operation to succeed, leading to a fully terminated friendship.
    ...
    // Assert that at least one end friendship calls succeeded.
    ...
    assertEquals(
      ...
      true,
      "At least one ending of friendship A<->B should succeed.",
    );
