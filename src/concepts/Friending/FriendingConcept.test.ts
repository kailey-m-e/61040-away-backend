...
Deno.test("Test Case 1 - operational principle", async () => {
    ...
    try {
        ...
        const validateFriendship1 = await friendingConcept.validateFriendship({user: userA, friend: userB});
        assertNotEquals("error" in validateFriendship1, true, "Friendship validation should not fail.");
        const validateFriendship2 = await friendingConcept.validateFriendship({user: userB, friend: userA});
        assertNotEquals("error" in validateFriendship2, true, "Friendship validation should not fail.");

        // 4. User ends friendship
        const endFriendship = await friendingConcept.endFriendship({user: userA, friend: userB});
        assertNotEquals("error" in endFriendship, true, "Ending friendship should not fail.");

        // 5. Friendship can no longer be validated
        const validateFriendship3 = await friendingConcept.validateFriendship({user: userA, friend: userB});
        assertEquals("error" in validateFriendship3, true, "Friendship validation should fail.");
        const validateFriendship4 = await friendingConcept.validateFriendship({user: userB, friend: userA});
        assertEquals("error" in validateFriendship4, true, "Friendship validation should fail.");
...
Deno.test("Test Case 2", async () => {
    ...
    try {
        ...
        const incoming = await friendingConcept._getIncomingRequests({user: userB});
...
Deno.test("Test Case 3", async () => {
    ...
    try {
        ...
        const rerequestFriend = await friendingConcept.requestFriend({user: userA, friend: userB});
        assertNotEquals("error" in rerequestFriend, true, "Friend request should not fail.");
...
Deno.test("Test Case 4", async () => {
    ...
    try {
        ...
        const unrequestFriend = await friendingConcept.unrequestFriend({user: userA, friend: userB});
        assertEquals("error" in unrequestFriend, true, "Friend request should fail.");

        // 2. User accepts friend that didn't request user
        const acceptFriend = await friendingConcept.acceptFriend({user: userA, friend: userB});
        assertEquals("error" in acceptFriend, true, "Accepting friend should fail.");

        // 3. User rejects friend that didn't request user
        const rejectFriend = await friendingConcept.rejectFriend({user: userA, friend: userB});
        assertEquals("error" in rejectFriend, true, "Rejecting friend should fail.");

        // 4. User rejects friend that didn't request user
        const endFriendship = await friendingConcept.endFriendship({user: userA, friend: userB});
        assertEquals("error" in endFriendship, true, "Ending friendship should fail.");
...
Deno.test("Test Case 5", async () => {
    ...
    try {
        ...
        const requestFriendBack = await friendingConcept.requestFriend({user: userB, friend: userA});
        assertEquals("error" in requestFriendBack, true, "Requesting friend back should fail.");

        // 3. Other user accepts
        await friendingConcept.acceptFriend({user: userB, friend: userA});

        // 4. Other user tries to request again
        const requestFriendAgain = await friendingConcept.requestFriend({user: userB, friend: userA});
        assertEquals("error" in requestFriendAgain, true, "Requesting friend again should fail.");

        // 5. Other user tries to accept
        const acceptFriend = await friendingConcept.acceptFriend({user: userB, friend: userA});
        assertEquals("error" in acceptFriend, true, "Accepting friendship that exists should fail.");

        // 6. Other user tries to reject
        const rejectFriend = await friendingConcept.rejectFriend({user: userB, friend: userA});
        assertEquals("error" in rejectFriend, true, "Rejecting friendship that exists should fail.");

        // 7. Other user tries to end
        const endFriendship = await friendingConcept.rejectFriend({user: userB, friend: userA});
        assertEquals("error" in endFriendship, true, "Ending friendship that doesn't exist should fail.");

        // 8. User tries to request itself
        const requestOneself = await friendingConcept.requestFriend({user: userA, friend: userA});
        assertEquals("error" in requestOneself, true, "User requesting itself should fail.");
...
Deno.test("Test Case 6", async () => {
    ...
    try {
        ...
        const incoming1 = await friendingConcept._getIncomingRequests({user: userB});
        ...
        const incoming2 = await friendingConcept._getIncomingRequests({user: userB});
        assertEquals(incoming2.length, 0, "UserB should have 0 incoming requests.");

        // 5. UserA validates friendship with userB
        const validateAB = await friendingConcept.validateFriendship({user: userA, friend: userB});
        assertNotEquals("error" in validateAB, true, "Validating friendship between userA and userB should not fail.");

        // 6. UserA validates friendship with userC
        const validateAC = await friendingConcept.validateFriendship({user: userC, friend: userB});
        assertNotEquals("error" in validateAC, true, "Validating friendship between userB and userC should not fail.");
